...
async function categoryFor(userId, name) {
...
router.get('/', async (request, response, next) => {
    try {
        ...
        if (query.search)
            where[Op.or] = [{ title: { [Op.like]: `%${query.search}%` } }, { description: { [Op.like]: `%${query.search}%` } }];
        ...
        const filtered = query.category ? tasks.filter((task) => task.Category?.name === query.category) : tasks;
...
router.get('/:id', async (request, response, next) => {
    try {
        ...
        return task ? response.json({ task: serializeTask(task) }) : response.status(404).json({ message: 'Task not found' });
    }
    catch (error) {
        return next(error);
    }
...
router.put('/:id', async (request, response, next) => {
    try {
        ...
        const updates = { ...input };
        if (input.category)
            updates.categoryId = (await categoryFor(request.user.id, input.category)).id;
        if (input.dueDate !== undefined)
            Object.assign(updates, splitDueDate(input.dueDate));
        if (input.status)
            updates.status = input.status === 'progress' ? 'in_progress' : input.status;
...
router.delete('/:id', async (request, response, next) => {
    try {
        const deleted = await Task.destroy({ where: { id: request.params.id, userId: request.user.id } });
        return deleted ? response.status(204).send() : response.status(404).json({ message: 'Task not found' });
    }
    catch (error) {
        return next(error);
    }
...
router.patch('/:id/status', async (request, response, next) => {
    try {
        const input = z.object({ status: statusSchema }).parse(request.body);
        const task = await ownedTask(request.user.id, request.params.id);
        if (!task)
            return response.status(404).json({ message: 'Task not found' });
        await task.update({ status: input.status === 'progress' ? 'in_progress' : input.status });
        const fullTask = await ownedTask(request.user.id, request.params.id);
        return response.json({ task: serializeTask(fullTask) });
    }
    catch (error) {
...
router.post('/:id/subtasks', async (request, response, next) => {
    try {
        const input = z.object({ title: z.string().trim().min(1).max(160), completed: z.boolean().default(false) }).parse(request.body);
        const task = await ownedTask(request.user.id, request.params.id);
        if (!task)
            return response.status(404).json({ message: 'Task not found' });
        const subtask = await Subtask.create({ taskId: task.id, title: input.title, isComplete: input.completed, position: await Subtask.count({ where: { taskId: task.id } }) });
        return response.status(201).json({ subtask: { id: String(subtask.id), title: subtask.title, completed: subtask.isComplete } });
    }
    catch (error) {
        return next(error);
    }
...
router.put('/:id/subtasks/:subtaskId', async (request, response, next) => {
    try {
        const input = z.object({ title: z.string().trim().min(1).max(160).optional(), completed: z.boolean().optional() }).parse(request.body);
        const task = await ownedTask(request.user.id, request.params.id);
        if (!task)
            return response.status(404).json({ message: 'Task not found' });
        const subtask = await Subtask.findOne({ where: { id: request.params.subtaskId, taskId: task.id } });
        if (!subtask)
            return response.status(404).json({ message: 'Subtask not found' });
        await subtask.update({ title: input.title, isComplete: input.completed });
        return response.json({ subtask: { id: String(subtask.id), title: subtask.title, completed: subtask.isComplete } });
    }
    catch (error) {
        return next(error);
    }
...
router.delete('/:id/subtasks/:subtaskId', async (request, response, next) => {
    try {
        ...
        if (!task)
            return response.status(404).json({ message: 'Task not found' });
        const deleted = await Subtask.destroy({ where: { id: request.params.subtaskId, taskId: task.id } });
        return deleted ? response.status(204).send() : response.status(404).json({ message: 'Subtask not found' });
    }
    catch (error) {
        return next(error);
    }
